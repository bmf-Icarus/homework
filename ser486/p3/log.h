...
#define LOG_H_INCLUDED
    ...
    /* write back one log entry to the eeprom if the eeprom is not busy.  For flushing the
    * cache during a watchdog event, this function should be called repeadedly to ensure
    * that all modified entries are written back.
    * for instance:  while (1) {log_update_noisr()}.
    */
